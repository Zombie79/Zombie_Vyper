  ...
  void DgusTFT::TimerEvent(timer_event_t event)  {
    ...
    switch (event) {
      case AC_timer_started: {
        live_Zoffset = 0.00; // reset print offset
...
#endif
          setMeshPoint(pos, constrain(currval - 0.02f, AC_LOWEST_MESHPOINT_VAL, 5));
        }
            ...
            switch (key_value)
            {
              ...
              case 4:
                ...
                GRID_LOOP(x, y) {
                  const xy_uint8_t pos { x, y };
                  const float currval = getMeshPoint(pos);
                  setMeshPoint(pos, constrain(currval + getZOffset_mm(), AC_LOWEST_MESHPOINT_VAL, 1));
                }
